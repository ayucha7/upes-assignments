...
int calcTime(int n,int *travelTimes,int *readyTimes,int *cancelTimes){
	int minTime = 0;
	for(int i=0;i<n;i++)
{
		...
		if(i != n-1)
{
			...
			if(minTime<=readyTimes[i]){
				...
				minTime += travelTimes[i];//adding traveltime to minimum time
			}else{
...
for(int i=0;i<n;i++){
	...
	cout<<calcTime(n,travelTimes,readyTimes,cancelTimes);
} 